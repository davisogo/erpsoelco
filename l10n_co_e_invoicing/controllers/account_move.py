...
class ElectronicInvoiceFunctions(http.Controller):
    ...
    @http.route('/approve-electronic-invoice/<string:token>/<string:approved>', type='http', auth='none')
    def approve_fe(self, token_id, rating, **kwargs):
        ...
        invoice_id.write({'fe_approved': rating_system})
        if rating == 'True':
            invoice_id.dian_document_lines.express_acceptation()
    ...
    @http.route(['/approve-electronic-invoice/<string:token>/refund_text'], type="http", auth="public", methods=['post'])
    def feedback_fe(self, token, **kwargs):
        invoice_id = request.env['account.move'].sudo().search([('approve_token', '=', token)], limit=1)
        if not invoice_id:
            return request.not_found()
        invoice_id.write({'refuse_text': kwargs.get('refuse_text')})
