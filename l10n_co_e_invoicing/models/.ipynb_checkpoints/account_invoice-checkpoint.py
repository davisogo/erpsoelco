...
class AccountInvoice(models.Model):
    ...
    operation_type = fields.Selection([('09', 'AIU'),
                                       ...
                                       ('32', 'Debit note without reference to invoices *')], string='Operation Type',
                                      default='10')
    ...
    status_dian_document = fields.Selection([('00', 'Procesado Correctamente'),
                                             ...
                                             ('other', 'Other')], string='Estado doc. DIAN',
                                            compute="_get_status_doc_dian", default=False, tracking=True)
            ...
            if origin_invoice.state in ('open', 'paid', 'posted'):
                for dian_document in origin_invoice.dian_document_lines:
                    if dian_document.state == 'done':
                        ...
                        msg1 = _(
                            'El documento DIAN de la factura %s no está procesado correctamente. Por favor, validar su estado en la DIAN.') % origin_invoice.name
