...
class ProductTemplateInvoiceAmount(models.Model):
    ...
    def _compute_get_customer_invoices_in_product(self):
        for record in self:
            if record.id:
                cust_invoice=self.env['account.move.line'].search([('product_id.product_tmpl_id','=',self.id),('move_id.move_type','=','out_invoice'),('exclude_from_invoice_tab','=',False)])
    ...
    def _compute_get_vendor_bills_in_product(self):
        for record in self:
            if record.id:
                vendor_bill=self.env['account.move.line'].search([('product_id.product_tmpl_id','=',self.id),('move_id.move_type','=','in_invoice'),('exclude_from_invoice_tab','=',False)])
